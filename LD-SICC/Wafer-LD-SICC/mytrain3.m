...
load Wafer_mergeData
load Wafer_TRAIN_X
load Wafer_TRAIN_Y
load Wafer_TEST_X
load Wafer_TEST_Y

mergeData=Wafer_mergeData;%每一类所有的子序列集合
TRAIN_X=Wafer_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=Wafer_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=Wafer_TEST_X;%TEST_X是测试集样本
TEST_Y=Wafer_TEST_Y;%TEST_Y是测试集标签
...
Wafer_means=[];%保存均值
Wafer_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    Wafer_means=[ Wafer_means;mean(mergeData(i).data) ];   
end
save Wafer_means.mat Wafer_means
for i=1:classnumber
    S=cov(mergeData(i).data);
    [ Wafer_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );    
end
save Wafer_inverse.mat Wafer_inverse
toc;

%%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=Wafer_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=Wafer_means; %means矩阵，数据集类别*维度
