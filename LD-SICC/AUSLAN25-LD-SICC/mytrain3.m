...
load AUSLAN25_mergeData
load AUSLAN25_TRAIN_X
load AUSLAN25_TRAIN_Y
load AUSLAN25_TEST_X
load AUSLAN25_TEST_Y

mergeData=AUSLAN25_mergeData;%每一类所有的子序列集合
TRAIN_X=AUSLAN25_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=AUSLAN25_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=AUSLAN25_TEST_X;%TEST_X是测试集样本
TEST_Y=AUSLAN25_TEST_Y;%TEST_Y是测试集标签
...
AUSLAN25_means=[];%保存均值
AUSLAN25_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    AUSLAN25_means=[ AUSLAN25_means;mean(mergeData(i).data) ];
end
save AUSLAN25_means.mat AUSLAN25_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
    [ AUSLAN25_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );    
end
save AUSLAN25_inverse.mat AUSLAN25_inverse
disp('inverse Over!');
%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=AUSLAN25_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=AUSLAN25_means; %means矩阵，数据集类别*维度
