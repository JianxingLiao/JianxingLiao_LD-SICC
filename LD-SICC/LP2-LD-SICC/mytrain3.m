...
mergeData=LP2_mergeData;%每一类所有的子序列集合
TRAIN_X=LP2_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=LP2_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=LP2_TEST_X;%TEST_X是测试集样本
TEST_Y=LP2_TEST_Y;%TEST_Y是测试集标签
...
LP2_means=[];%保存均值
LP2_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
...
Ma=LP2_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=LP2_means; %means矩阵，数据集类别*维度
