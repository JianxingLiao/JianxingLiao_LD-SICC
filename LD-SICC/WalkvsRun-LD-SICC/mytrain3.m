...
load WalkvsRun_mergeData
load WalkvsRun_TRAIN_X
load WalkvsRun_TRAIN_Y
load WalkvsRun_TEST_X
load WalkvsRun_TEST_Y

mergeData=WalkvsRun_mergeData;%每一类所有的子序列集合
TRAIN_X=WalkvsRun_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=WalkvsRun_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=WalkvsRun_TEST_X;%TEST_X是测试集样本
TEST_Y=WalkvsRun_TEST_Y;%TEST_Y是测试集标签
...
WalkvsRun_means=[];%保存均值
WalkvsRun_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    WalkvsRun_means=[ WalkvsRun_means;mean(mergeData(i).data) ];
end
save WalkvsRun_means.mat WalkvsRun_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
%         [ WalkvsRun_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );
    [ WalkvsRun_inverse{1,i},W{1,i} ] = glasso( S, lambda );
end
save WalkvsRun_inverse.mat WalkvsRun_inverse
disp('inverse Over!');
%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=WalkvsRun_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=WalkvsRun_means; %means矩阵，数据集类别*维度
