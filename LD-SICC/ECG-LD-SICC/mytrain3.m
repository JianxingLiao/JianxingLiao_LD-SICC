...
mergeData=ECG_mergeData;%每一类所有的子序列集合
TRAIN_X=ECG_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=ECG_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=ECG_TEST_X;%TEST_X是测试集样本
TEST_Y=ECG_TEST_Y;%TEST_Y是测试集标签
...
ECG_means=[];%保存均值
ECG_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    ECG_means=[ ECG_means;mean(mergeData(i).data) ];
end
save ECG_means.mat ECG_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
    [ ECG_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );    
end
save ECG_inverse.mat ECG_inverse
disp('inverse Over!');
%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=ECG_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=ECG_means; %means矩阵，数据集类别*维度
