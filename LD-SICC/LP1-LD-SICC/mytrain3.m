...
mergeData=LP1_mergeData;%每一类所有的子序列集合
TRAIN_X=LP1_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=LP1_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=LP1_TEST_X;%TEST_X是测试集样本
TEST_Y=LP1_TEST_Y;%TEST_Y是测试集标签
...
LP1_means=[];%保存均值
LP1_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    LP1_means=[ LP1_means;mean(mergeData(i).data) ];  
end
save LP1_means.mat LP1_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
    [ LP1_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );    
end
save LP1.mat LP1_inverse
disp('inverse Over!');
%%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=LP1_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=LP1_means; %means矩阵，数据集类别*维度
