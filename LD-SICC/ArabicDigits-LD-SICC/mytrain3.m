...
load ArabicDigits_mergeData
load ArabicDigits_TRAIN_X
load ArabicDigits_TRAIN_Y
load ArabicDigits_TEST_X
load ArabicDigits_TEST_Y

mergeData=ArabicDigits_mergeData;%每一类所有的子序列集合
TRAIN_X=ArabicDigits_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=ArabicDigits_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=ArabicDigits_TEST_X;%TEST_X是测试集样本
TEST_Y=ArabicDigits_TEST_Y;%TEST_Y是测试集标签
...
ArabicDigits_means=[];%保存均值
ArabicDigits_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    ArabicDigits_means=[ ArabicDigits_means;mean(mergeData(i).data) ];
end
save ArabicDigits_means.mat ArabicDigits_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
        [ ArabicDigits_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );
%     [ ArabicDigits_inverse{1,i},W{1,i} ] = glasso( S, lambda );
end
save ArabicDigits_inverse.mat ArabicDigits_inverse
disp('inverse Over!');
%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=ArabicDigits_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=ArabicDigits_means; %means矩阵，数据集类别*维度
...
fprintf('-----------------------------------------\n');ltest_accuracy_old=test_accuracy;%保存上一次求解的准确率
