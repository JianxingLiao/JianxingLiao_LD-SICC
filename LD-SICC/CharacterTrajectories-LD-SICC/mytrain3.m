...
load CharacterTrajectories_mergeData
load CharacterTrajectories_TRAIN_X
load CharacterTrajectories_TRAIN_Y
load CharacterTrajectories_TEST_X
load CharacterTrajectories_TEST_Y

mergeData=CharacterTrajectories_mergeData;%每一类所有的子序列集合
TRAIN_X=CharacterTrajectories_TRAIN_X;%TRAIN_X是训练集样本
TRAIN_Y=CharacterTrajectories_TRAIN_Y;%TRAIN_Y是训练集标签
TEST_X=CharacterTrajectories_TEST_X;%TEST_X是测试集样本
TEST_Y=CharacterTrajectories_TEST_Y;%TEST_Y是测试集标签
...
CharacterTrajectories_means=[];%保存均值
CharacterTrajectories_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    CharacterTrajectories_means=[ CharacterTrajectories_means;mean(mergeData(i).data) ];  
end
save CharacterTrajectories_means.mat CharacterTrajectories_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
    [ CharacterTrajectories_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );    
end
save CharacterTrajectories_inverse.mat CharacterTrajectories_inverse
disp('inverse Over!');
%%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=CharacterTrajectories_inverse;%保存了每个类别的稀疏逆协方差
Me=Ma;%保存上一次迭代的逆协方差矩阵
saveresults=[];%保存最终的结果
means=CharacterTrajectories_means; %means矩阵，数据集类别*维度
