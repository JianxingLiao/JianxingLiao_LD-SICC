...
CMUsubject16_means=[];%保存均值
CMUsubject16_inverse=cell(1,classnumber);%保存稀疏逆协方差
for i=1:classnumber
    CMUsubject16_means=[ CMUsubject16_means;mean(mergeData(i).data) ];  
end
save CMUsubject16_means.mat CMUsubject16_means
disp('means Over!');
for i=1:classnumber
    S=cov(mergeData(i).data);
    [ CMUsubject16_inverse{1,i},W{1,i} ] = graphicalLasso( S, lambda, maxIt, tol );    
    %[ CMUsubject16_inverse{1,i},W{1,i} ] = glasso( S, lambda );
end
save CMUsubject16_inverse.mat CMUsubject16_inverse
disp('inverse Over!');
%%---通过LogDet散度进一步优化代表每个类别的稀疏逆协方差---%%
Ma=CMUsubject16_inverse;%保存了每个类别的稀疏逆协方差
